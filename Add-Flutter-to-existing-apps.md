...
left part of the main view and then select the General tab.

In the Embedded Binaries section, click the + and select `Flutter.framework`
from the pop-up dialog. Click Add.
...
In the `onViewDidLoad` method of `ViewController.m`, add the following:
```objective-c
- (void)viewDidLoad {
  [super viewDidLoad];
