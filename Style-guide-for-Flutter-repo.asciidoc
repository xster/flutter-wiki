...
Sample code should go in a section of the documentation that begins with `{@tool sample}`, and ends with `{@end-tool}`. This will then be checked by automated tools, and extracted and formatted for display on the API documentation web site [docs.flutter.io](https://docs.flutter.io).
...
When referencing an argument, use square brackets if the argument corresponds to a property as well, otherwise
use backticks.
