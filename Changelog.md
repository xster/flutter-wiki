...
* [#34545](https://github.com/flutter/flutter/issues/34545) In order to keep errors from getting inadvertently swallowed in `FutureBuilder` and `StreamBuilder`, the following changes were made to those APIs ([announcement](https://groups.google.com/forum/#!topic/flutter-announce/H6Od0QdsdrI)):
  ...
  * `AsyncSnapshot.requireData` was removed. Callers who are using this can update their code by accessing `AsyncSnapshot.data` instead, which now behaves the way that `requireData` did.
  ...
  * The `initialData` argument to the `FutureBuilder()` constructor is now deprecated and will be removed in a future release. Callers who are using `FutureBuilder.initialData` should instead check the snapshot's connection state in their builder method:
