...
| _20:00_ | [@tvolkert](https://github.com/tvolkert) manages to reconstruct [@timsneath's](https://github.com/timsneath) environment setup and confirms [@jason-simmons'](https://github.com/jason-simmons) theory.  The team adds a section to the wiki covering this case for users who may run into it. |
...
* In the “channel” command, we weren’t fetching updated refs from GitHub before checking out the branch.  This meant that if a user had switched to the beta branch in the past, then switched back to the alpha/dev branch as their primary channel, their beta Git head would be pointing to a commit in December.  Net result: we had to recommend that users run `flutter channel beta && flutter upgrade`, which is clunky.
